...
2. **`code-simplifier` agent — the simplify phase.** `subagent_type: sontek-skills:code-simplifier`, same scope. Simplification is apply-oriented (it proposes refactors to make, not findings to report), so it lives here in the apply loop rather than inside `review-code`. Returns a per-file change report.
