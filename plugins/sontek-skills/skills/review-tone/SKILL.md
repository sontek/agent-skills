...
- Active voice; only simple tenses (no present perfect, no modal stacks, no periphrastic future: "it may help to improve" → "this improves"; "is about to enable" → "enables").
...
### H. Cut modifiers that don't change the claim

A qualifier earns its place only if removing it changes what's true. "The **module** docstring says" and "the docstring says" claim the same thing when only one docstring is in view — drop "module". Words like *already, own, explicitly, anywhere in this stack,* and *especially* often survive a rewrite as emphasis rather than information: "your **own** guardrail" and "your guardrail" mean the same thing; "no alarm exists **anywhere in this stack**" and "no alarm exists" claim the same thing. Test each modifier by deleting it: if the sentence still makes the identical claim, the modifier was filler.

This rule has no script. The em-dash stripper and `ste_lint.py` check named words and grammar, not whether a modifier adds information — a draft can score 0/100w and still carry several of these.

### I. Speak in your own voice, not the system's

When the writer is part of the team or system being described, say so instead of routing the claim through an inanimate subject: "we have `X` conventions elsewhere" beats "the repo already has `X` conventions elsewhere"; "I don't see an alarm for this" beats "no alarm exists for this in the stack." Both versions in each pair are already grammatically active, so Layer 1's active-voice rule will not catch the second one — the sentence has a real, known actor (you), just hidden behind an impersonal noun instead of named. If you're the one making the observation, say "we" or "I."
...
4. Any present perfect ("has/have received"), modal stack, or periphrastic future ("is about to", "is going to")? Use a simple tense.
...
Both scripts check Rule A and the lintable parts of Layer 1 (now including periphrastic future in `complex_tense`). Rules B through I (filler, labels, editorializing, jargon, **bottom-line-first and conciseness**, AI attribution, redundant modifiers, direct voice) have no script — nothing fails loudly if you skip them, so it is easy to run the two commands, see a good number, and stop without re-reading the draft against those eight. Do the re-read every time, not only when a script flags something. A 0/100w score is consistent with a draft that still has a full editing pass left in it.

Run this checklist after the scripts pass, on the exact final text, not from memory of an earlier draft:
