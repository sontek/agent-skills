...
- **Name the observable effect, not the runtime mechanism.** Say what the author or operator would *see*, not the plumbing that produces it. "A failed run shows a traceback, not a `statusCode` body" is what they act on; "the handler re-raises to register a FunctionError" narrates internals they didn't ask about. Reach for the mechanism only when the fix depends on understanding it.
- **When the finding is a mismatch, say which side is right and which one changes — and lead with it.** Code-vs-doc, test-vs-behavior, comment-vs-code: a comment that only *describes the disagreement* leaves the author guessing what you want. State the verdict first. "The code raising here is correct; it's the runbook line that's stale" tells them the fix in one read; "the runbook says 500 but the code raises" makes them reverse-engineer whether you want the code changed or the doc. Don't make them guess which side you're defending. Decide which side is the source of truth and which drifted from it: a published contract or spec is authoritative over code that violates it, and a deliberate code change (often flagged by a diff comment or an edited line) is authoritative over a doc, comment, or test that wasn't updated to match. Make that call and recommend fixing the side that drifted; don't punt with "which one is correct?"
