...
Invoke via the Task tool with `subagent_type: sontek-skills:security-auditor`. The agent has isolated context, so the prompt must be self-contained. Include:
