...
Use this instead of Claude Code's built-in `/simplify`, which spawns generic review agents without the curated rubric. A bare `/simplify` runs the built-in; this skill is reached by natural language ("simplify this", "clean this up") or by its plugin-qualified name `/sontek-skills:simplify-code`.
...
Invoke via the Task tool with `subagent_type: sontek-skills:code-simplifier`. The prompt must be self-contained:
