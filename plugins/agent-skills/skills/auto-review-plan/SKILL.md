...
description: Automatically iterate review-plan against a plan file until no safe blocking edits remain. Use when user wants to "auto-review-plan", "auto-fix the plan", tighten a plan without staying in the loop for each finding, or harden `IMPLEMENTATION_PLAN_*.md` / `REFACTOR_PLAN_*.md` before implementation. Auto-applies small, concrete blocking edits; batches structural recommendations for a single end-of-run approval pass.
---
...
This skill is the plan-side analog of `auto-review`. It tightens a plan document *before* implementation starts, rather than cleaning code after.
...
- Hardening an `IMPLEMENTATION_PLAN_*.md` or `REFACTOR_PLAN_*.md` before starting work

## When NOT to use

- Single-shot plan critique only — use `review-plan` directly
- In-conversation plan-mode draft — the loop needs a file to edit in place; run `review-plan` once and hand the draft back to the user to revise before `ExitPlanMode`
...
The skill requires a plan file at a stable path so it can edit it in place and re-review. In order of preference:
...
5. **Apply plan edits.** For each `auto-apply` fix, edit the plan file in place. Use `Edit` with narrow `old_string` / `new_string` anchored to the finding's section so unrelated text isn't disturbed. After each edit, re-read the plan file to confirm the change landed.
...
- **Edit failure loop** — If three consecutive edit attempts fail to land (e.g., `old_string` no longer matches after drift), stop and escalate with the current log.
...
```
{plan_file}:{section_or_phase}|{category}|{short_summary_slug}
```
...
This skill orchestrates `review-plan` only. For each round, invoke `review-plan` with the target plan path and apply its output per the policy above. Do not re-implement `review-plan`'s delegation to `senior-engineer` and `product-manager` — call the skill and consume its structured output.
