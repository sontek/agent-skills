...
- **Non-atomic read-modify-write over a row set (TOCTOU).** Enumerated mechanically by investigation step 2c — a fetch-keys-then-mutate-by-key sequence whose mutating statement drops the original state predicate, so a row that transitions out of the predicate in the gap is still mutated/deleted. See `references/patterns.md` ("Non-atomic read-modify-write over a row set (TOCTOU)").
